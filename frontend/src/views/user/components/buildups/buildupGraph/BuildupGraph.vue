<template>
  <div class="plotly-graph">
    ...
    <div class="warning" v-if="graphType === 'lifeCycleComparison' && selectedIndicesLength < 2">
      <p>Please select at least 2 buildups to compare.</p>
    </div>
...
// Subcomponents
import BuildupGraphTypeSelector from '@/views/user/components/buildups/buildupGraph/BuildupGraphTypeSelector.vue';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const xAxisColumn = ref<string>('Name'); // Default X-axis column
    ...
    const displayModes = [
      { key: 'filtered', label: 'Filtered Buildups' },
      { key: 'selected', label: 'Selected Buildups' },
    ...
    const sliceBy = ref<string>('Status'); // What to slice the pie by
