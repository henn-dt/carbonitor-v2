...
# first you filter the impacts that you want to display
# those impacts are added as column to the table, and generate graphs. 
# then you filter the life cycle stages that are interesting
# this is based on aggregation: user will filter e.g. "Operation", which is all Life Cycle Stages in B (B1, B2-5, B6, B7)
# then we display in the table the sum of all life cycle stages that are selected
# graph has the option to color portion of the impact by life cycle stage
...
class ReferenceSourceForAssembly2(BaseModel):
    ...
    type: Type3   # this is a reference value, only the link to assembly
    uri: str
...
class ReferenceSourceForProduct2(BaseModel):
    ...
    format: Optional[str] = None
    overrides: Optional[Dict[str, Any]] = None
    type: Type8     # this is a reference to an epd
    uri: str        # address of referenced epd - id + source
    version: Optional[str] = None
...
class ConversionUnitDescription(Enum):
    KG_PER_M = "kilograms per meter"
    KG_PER_M3 = "kilograms per cubic metre"
    KG_PER_M2 = "kilograms per square meter"
    M = "meters"
    KG_PER_PCS = "weight per piece"


class ConversionConfig:
    MAPPINGS = {
        ConversionType.LINEAR_DENSITY: {
            "to": Unit.kg,
            "unit": ConversionUnit.KG_PER_M,
            "unit_description": ConversionUnitDescription.KG_PER_M,
        },
        ConversionType.GROSS_DENSITY: {
            "to": Unit.kg,
            "unit": ConversionUnit.KG_PER_M3,
            "unit_description": ConversionUnitDescription.KG_PER_M3,
        },
        ConversionType.GRAMMAGE: {
            "to": Unit.kg,
            "unit": ConversionUnit.KG_PER_M2,
            "unit_description": ConversionUnitDescription.KG_PER_M2,
        },
        ConversionType.LAYER_THICKNESS: {
            "to": Unit.m,
            "unit": ConversionUnit.M,
            "unit_description": ConversionUnitDescription.M,
        },
        ConversionType.BULK_DENSITY: {
            "to": Unit.kg,
            "unit": ConversionUnit.KG_PER_M3,
            "unit_description": ConversionUnitDescription.KG_PER_M3,
        },
        ConversionType.WEIGHT_PER_PIECE: {
            "to": Unit.kg,
            "unit": ConversionUnit.KG,
            "unit_description": ConversionUnitDescription.KG_PER_PCS,
        },
    }
...
class Conversion(BaseModel):
    ...
    meta_data: Optional[ConversionMetaData] = Field(None, alias="metaData")
    to: Unit
    value: float
...
class EPD(BaseModel):
    ...
    id: str  # product_id random guid
    impacts: Dict[str, Dict[str, Optional[float]]]  # this is the most important part input
    comment: Optional[str] = None  # not mendatory optional a string
    conversions: Optional[List[Conversion]] = (None)  # this can be given by users or non this is also important implement the same epdx structure of Conversion
    declared_unit: Optional[Unit] = Field(None, alias="declaredUnit")  # user must give
    format_version: Optional[str] = Field(None, alias="formatVersion")  # automatically start from 0 by create, if update increment as 0.1
    location: Optional[Country] = None  # user chose from dropdown list of counteries
    meta_data: Optional[Dict[str, Any]] = Field(None, alias="metaData")  # will be none      # we can store extra properties here, for instance to serde product dto
    published_date: Optional[date] = Field(None, alias="publishedDate")  # default now but can be defined by the user
    reference_service_life: Optional[int] = Field(None, alias="referenceServiceLife", ge=0)  # user defined default none
    source: Optional[Source] = None  # user defined --> 'custom'
    standard: Optional[Standard] = None  # dropdown for user 3 options
    subtype: Optional[SubType] = None  # dropdown for user with 4 options
    valid_until: Optional[date] = Field(None, alias="validUntil")  # default value= plus 5 years or user defined
    version: Optional[str] = None # 1.1


# change product entity according to this
...
class ReferenceSourceForImpactDataSource1(EPD):    #ihnerits from EPD, *not* product!
    model_config = ConfigDict(
        populate_by_name=True,
    )
...
class ReferenceSourceForImpactDataSource2(TechFlow):   # ihnerits from tech flow, which we don´t use
    model_config = ConfigDict(
        populate_by_name=True,
    )
    type: Type4    # means reference
...
class ReferenceSourceForProduct1(BaseModel):
    ...
    meta_data: Optional[Dict[str, Any]] = Field(None, alias="metaData")     # we store mapping entry in meta_data, "mapping_filter_name" : "str", "mapping_filter_id" : "str"
    name: str
    quantity: float
    reference_service_life: int = Field(..., alias="referenceServiceLife", ge=0)
    results: Optional[Dict[str, Any]] = None
    transport: Optional[List[Transport]] = None
    type: Type7   # means actual
    unit: Unit
...
class ReferenceSourceForAssembly1(BaseModel):
    ...
    quantity: float
    results: Optional[Dict[str, Any]] = None   #here come the product amounts values
    type: Type2   # this is actual data
    unit: Unit
    """
    sample use:
    results = {"mapping_id" : {"quantity" : 100}}
    """
